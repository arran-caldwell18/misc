...
Generalize data (we can have a file of data instead so we can easly interchange trainign data)

Make testing cleaner and more effiecient (with functions (which is well overdue))

Parameter tuning (includes hyper parameters)

Make the Network scalable

Visulatize predictions *(cuz why the hell not)

MAIN IMPORV:
    Wrap Everything In A Class!
     
    (better for everyone single thing almsot)
"""
try:
    import pandas as pd
    import ctypes
...
#   trainign data import / clean
    training_data = pd.read_csv(r"/home/arranthegreat/Desktop/PYTHON/neural network from scratch/v3/training_data.csv",header=None) #honeslt can not remeber if i even need to state that
except IOError:
...
class network_xor_model:
    def __init__(self, lib = r"/home/arranthegreat/Desktop/PYTHON/neural network from scratch/v3/logic.so"):
        try:
    ...
    def train(self, epochs=50_000, lr=0.1):
        #training the XORnet 
        self.lib.train_model(self.w, self.b, epochs, lr)
